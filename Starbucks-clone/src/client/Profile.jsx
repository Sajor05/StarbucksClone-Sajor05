...
const Purchase = () => {
  ...
  return (
    <div className="p-5">
      <PurchaseTitle />
      <div className="flex flex-row gap-5 mt-5 h-66 overflow-x-auto">
        {purchaseHistory.length > 0 ? (
          purchaseHistory.map((c, i) => (
            <div key={i} className="p-3 border border-gray-400 rounded-lg">
              <span className="text-[19px] font-semibold">
                Compra hecha el {c.date}
              </span>
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <ul className="flex flex-col gap-2 overflow-y-auto">
                    {c.items.map((i, j) => (
                      <li key={j} className="font-light list-disc">
                        {i.title}
                      </li>
                    ))}
                  </ul>
          ...
          <div>
            <span className="font-light">Aun no se han realizado compras</span>
          </div>
        )}
      </div>
    </div>
