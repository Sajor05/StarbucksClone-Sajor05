          ...
          <div className="cart-total-price-container text-center mt-5">
            {cart.length > 0 ?
              <span className="cart-total-price text-[22px] font-semibold">
                Total 
                ${cart.reduce((total, item) => total + (item.price * item.currentItemCount),0)}
              </span> : ""}
          </div>      
        </div>
...
const BuyButton = () => {
    ...
    return(
          <div className="flex justify-center mt-3">
              <button onClick={() => handleBuy()} className="bg-[#1e3932] text-center p-2 rounded-full cursor-pointer">
